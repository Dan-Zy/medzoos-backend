...
const normalizeDayName = (day) => {
  ...
  return DAY_ALIAS_MAP[key] || (DAYS.includes(day) ? day : null);
};
...
const normalizeWeeklySchedule = (slots) => {
  if (Array.isArray(slots) && slots.length && typeof slots[0] === 'object' && slots[0]?.day) {
    const mapByDay = {};
    for (const entry of slots) {
      const normDay = normalizeDayName(entry.day);
      if (normDay) {
        mapByDay[normDay] = coerceDaySlots(entry.slots);
      }
    }
    return DAYS.map((day) => ({
      day,
  ...
  if (slots && typeof slots === 'object' && !Array.isArray(slots)) {
    if (Array.isArray(slots.weekly)) return normalizeWeeklySchedule(slots.weekly);
    if (Array.isArray(slots.schedule)) return normalizeWeeklySchedule(slots.schedule);
  }
  if (Array.isArray(slots) && slots.length && typeof slots[0] === 'string') {
    const coerced = coerceDaySlots(slots);
    return DAYS.map((day) => ({ day, slots: coerced }));
  }
  if (typeof slots === 'string' && slots.trim()) {
    const coerced = coerceDaySlots(slots.trim());
    return DAYS.map((day) => ({ day, slots: coerced }));
  }

  const defaultSlots = ['09:00 AM - 01:00 PM', '05:00 PM - 08:00 PM'];
  return DAYS.map((day) => ({
    day,
    slots: ['Saturday', 'Sunday'].includes(day) ? [] : defaultSlots,
  }));
};
