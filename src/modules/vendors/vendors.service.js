...
async function registerVendor(data) {
  ...
  if (cleanLicense && cleanLicense.toUpperCase() !== 'PENDING') {
    ...
    if (existingWithLicense) {
      throw new AppError(
        `Pharmacy license number '${cleanLicense}' is already registered to another pharmacy (${existingWithLicense.business_name || existingWithLicense.name}).`,
        400,
      );
    }
  }
...
async function getDashboardStats(vendorId) {
  ...
  const pendingOrders = orders.filter((order) =>
    ['pending', 'NEW', 'awaiting_payment', 'AWAITING_PAYMENT'].includes(order.status)
  ).length;
  const processingOrders = orders.filter((order) =>
    ['processing', 'shipped', 'ACCEPTED', 'PREPARING', 'READY_FOR_PICKUP', 'OUT_FOR_DELIVERY', 'out_for_delivery'].includes(order.status)
  ).length;
