...
const createProduct = async (vendorId, data) => {
  ...
  await recordAuditEntry({
    ...
    details: {
      ...
      controlled_medicine: Boolean(product.controlled_medicine),
    },
  });
  ...
  if (submit) {
    try {
      ...
      await inboxEvents.productSubmittedForReview({
        product,
        vendorName: vendor?.business_name,
      });
    } catch {
      // Notification dispatch should not block product creation
    }
  }
...
const listVendorProducts = async (vendorId, query = {}) => {
  ...
  if (query.approval_status) {
    andConditions.push({ approval_status: query.approval_status });
  }
  if (query.prescription_required === 'true') {
    andConditions.push({ prescription_required: true });
  } else if (query.prescription_required === 'false') {
    andConditions.push({ prescription_required: false });
  }
  ...
  if (listingFilter === 'OUT_OF_STOCK' || stockFilter === 'OUT_OF_STOCK') {
    andConditions.push({
      OR: [
        ...
        { inventory: { is: { available_quantity: { lte: 0 } } } },
      ],
    });
  } else if (listingFilter) {
    andConditions.push({ listing_status: listingFilter });
  }
    ...
    andConditions.push({
      AND: [
        { stock: { gt: 0 } },
        {
          OR: [
            { inventory: null },
            { inventory: { is: { available_quantity: { gt: 0 } } } },
          ],
        },
      ],
    });
  ...
  const filtered = items.map(withComputed);
...
const updateProduct = async (id, vendorId, data) => {
  ...
  if (nextApproval === 'pending_review') {
    try {
      const vendor = await prisma.vendor.findUnique({
        where: { id: vendorId },
        select: { business_name: true },
...
const reviewProduct = async (productId, accountId, approvalStatus, reviewNote = null) => {
  ...
  const isControlled = Boolean(reviewedProduct.controlled_medicine);

  await vendorNotificationsService.createVendorNotification({
    vendorId: reviewedProduct.vendor_id,
    type: isApproved ? 'product_approved' : 'product_rejected',
    title: isApproved
      ? (isControlled ? 'Controlled medicine approved' : 'Product approved')
      : 'Product review update',
    message: isApproved
      ? `${reviewedProduct.name} is approved${isControlled ? ' after administrative validation' : ''} and can now be listed.`
  ...
  await inbox.notify({
    ...
    recipientId: reviewedProduct.vendor_id,
    type: isApproved ? 'product_approved' : 'product_rejected',
    title: isApproved
      ? (isControlled ? 'Controlled medicine approved' : 'Product approved')
      : 'Product review update',
    message: isApproved
      ? `${reviewedProduct.name} has been approved by admin and is now active.`
      : `${reviewedProduct.name} requires changes: ${reviewNote || 'Please review administrative feedback.'}`,
