...
function hasHealthInfo(member) {
  if (member.health_score != null) return true;
  if (member.blood_group || member.height_cm || member.weight_kg) return true;
  if (member.medicines && member.medicines.length > 0) return true;
  if (member.vaccinations && member.vaccinations.length > 0) return true;
  if (member.vitals && member.vitals.length > 0) return true;
  if (member.lab_reports && member.lab_reports.length > 0) return true;
  const mp = member.medical_profile || {};
  if (
    (mp.conditions && mp.conditions.length > 0) ||
    (mp.surgeries && mp.surgeries.length > 0) ||
    (mp.familyHistory && mp.familyHistory.length > 0) ||
    (mp.allergies && Object.values(mp.allergies).some((arr) => Array.isArray(arr) && arr.length > 0)) ||
    (mp.lifestyle && Object.keys(mp.lifestyle).length > 0)
  ) {
    return true;
  }
  return false;
...
function computeHealthScore(member) {
  ...
  if (!hasHealthInfo(member)) return null;
...
function buildWeeklySummary(members) {
  ...
  const validScores = memberSummaries.map((m) => m.health_score).filter((s) => s !== null);
  const overallScore = validScores.length
    ? Math.round(validScores.reduce((s, score) => s + score, 0) / validScores.length)
    : null;
