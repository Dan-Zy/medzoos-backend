...
async function getFamily(userId) {
  ...
  const scores = vault.members
    .map((m) => computeHealthScore(m))
    .filter((s) => s !== null);
  return {
    ...vault,
    members: vault.members.map(mapMember),
    dashboard: buildDashboard(vault.members),
    family_health_score: scores.length
      ? Math.round(scores.reduce((s, val) => s + val, 0) / scores.length)
      : null,
  };
}
