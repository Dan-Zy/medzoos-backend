...
const removeFromCart = async (userId, itemId) => {
  ...
  cart.items = cart.items.filter(item => 
    item.product_id !== itemId && item.id !== itemId && item.productId !== itemId
  );
  cart.total = cart.items.reduce((acc, item) => acc + (item.price * item.quantity), 0);
...
const updateCartItemQuantity = async (userId, itemId, quantity) => {
  ...
  const productId = cart.items[existingItemIndex].product_id || cart.items[existingItemIndex].id || itemId;
  const product = await prisma.product.findUnique({ where: { id: productId } });

  if (quantity <= 0) {
    cart.items.splice(existingItemIndex, 1);
  } else {
    cart.items[existingItemIndex].quantity = product ? Math.min(quantity, product.stock) : quantity;
  }
