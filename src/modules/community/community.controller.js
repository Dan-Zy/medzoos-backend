...
exports.reportPost = catchAsync(async (req, res) => {
  const data = await communityService.reportPost(req.params.id, req.user.id, req.body);
  res.status(200).json({ status: 'success', ...data });
});

exports.reportComment = catchAsync(async (req, res) => {
  const data = await communityService.reportComment(req.params.id, req.user.id, req.body);
  res.status(200).json({ status: 'success', ...data });
});

exports.blockUser = catchAsync(async (req, res) => {
  const data = await communityService.blockUser(req.params.id, req.user.id);
  res.status(200).json({ status: 'success', ...data });
});

exports.deletePost = catchAsync(async (req, res) => {
  const data = await communityService.deletePost(req.params.id, req.user.id);
  res.status(200).json({ status: 'success', ...data });
});
