...
function mapPost(post, viewerId) {
  ...
  return {
    ...
    authorAvatar: post.is_anonymous ? null : (post.user?.avatar || null),
    ...
    commentList: (post.comments || []).map(c => ({
      ...
      authorAvatar: c.is_anonymous ? null : (c.user?.avatar || null),
...
async function getPosts(query = {}, viewerId = null) {
  ...
  const posts = await prisma.communityPost.findMany({
    where,
    include: {
      user: { select: { id: true, name: true, is_verified: true, avatar: true } },
      likes: viewerId ? { where: { user_id: viewerId } } : false,
      comments: {
        take: 5,
        orderBy: { created_at: 'asc' },
        include: { user: { select: { id: true, name: true, is_verified: true, avatar: true } } },
      },
...
async function createPost(userId, body) {
  ...
  const isMediaPost = Boolean(body.image_url || body.video_url);

  if (content) {
    const moderation = moderatePostContent(content, isMediaPost);
    if (!moderation.approved) {
      throw new AppError(moderation.reason, 400);
    }
  } else if (!isMediaPost) {
    throw new AppError('Write at least 10 characters or add a photo/video', 400);
  }
...
async function reportPost(postId, userId, { reason, details }) {
  const post = await prisma.communityPost.findUnique({ where: { id: postId } });
  if (!post) throw new AppError('Post not found', 404);

  console.log(
    `[UGC MODERATION REPORT] User ${userId} reported post ${postId} for ${reason}: ${details || 'No details'}`,
  );

  return {
    success: true,
    message:
      'Thank you for reporting. This content has been flagged for moderation review and hidden from your feed.',
    postId,
    status: 'flagged_for_review',
  };
}

async function reportComment(commentId, userId, { reason, details }) {
  const comment = await prisma.communityComment.findUnique({ where: { id: commentId } });
  if (!comment) throw new AppError('Comment not found', 404);

  console.log(
    `[UGC MODERATION REPORT] User ${userId} reported comment ${commentId} for ${reason}: ${details || 'No details'}`,
  );

  return {
    success: true,
    message: 'Thank you for reporting. Our moderation team reviews flagged comments within 24 hours.',
    commentId,
  };
}

async function blockUser(blockedUserId, userId) {
  if (blockedUserId === userId) throw new AppError('You cannot block yourself', 400);

  console.log(`[UGC BLOCK] User ${userId} blocked user ${blockedUserId}`);

  return {
    success: true,
    message: 'User blocked. You will no longer see posts or comments from this user.',
    blockedUserId,
  };
}
...
async function deleteComment(commentId, userId) {
  ...
  return { success: true, message: 'Comment deleted successfully', commentId };
}
