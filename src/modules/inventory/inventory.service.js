...
async function ensureInventory(tx, vendorId, productId, available = null) {
  ...
  const stockFromProduct = Number(product?.stock) || 0;

  let initialQty = stockFromProduct;
  if (available !== null && available !== undefined && Number(available) > 0) {
    initialQty = Number(available);
  }
  ...
  return tx.productInventory.create({
    data: {
      ...
      available_quantity: Math.max(0, initialQty),
    },
  });
}
...
const applyOrderStockChange = async (tx, { vendorId, productId, quantity, type, referenceId, performedBy }) => {
  await ensureInventory(tx, vendorId, productId);
  const inventory = await tx.productInventory.findUnique({ where: { product_id: productId } });
  let availableDelta = 0;
  let reservedDelta = 0;
  let soldDelta = 0;
