...
const assignCollector = catchAsync(async (req, res) => {
  const booking = await labTestsService.assignCollector(
    req.user.id,
    req.params.id,
    req.body
  );
  sendResponse(res, 200, { booking }, 'Sample collector assigned successfully');
});

const markPaymentReceived = catchAsync(async (req, res) => {
  const booking = await labTestsService.markPaymentReceived(req.user.id, req.params.id);
  sendResponse(res, 200, { booking }, 'Cash payment marked as received');
});

const createLabTest = catchAsync(async (req, res) => {
  const test = await labTestsService.createLabTest(req.body);
  sendResponse(res, 201, { test }, 'Lab test created successfully');
});

const updateLabTest = catchAsync(async (req, res) => {
  const test = await labTestsService.updateLabTest(req.params.id, req.body);
  sendResponse(res, 200, { test }, 'Lab test updated successfully');
});

const deleteLabTest = catchAsync(async (req, res) => {
  const test = await labTestsService.deleteLabTest(req.params.id);
  sendResponse(res, 200, { test }, 'Lab test deactivated successfully');
});
