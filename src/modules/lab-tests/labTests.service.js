...
const updateBookingStatus = async (labPartnerId, bookingId, rawStatus, note) => {
  ...
  // Cash at lab counter / home collection: when sample is collected or booking is progressing, reception/collector has taken payment
  const method = String(booking.payment_method || '').toLowerCase().replace(/[_\s-]+/g, ' ');
  const isCash =
    !method ||
    method === 'cod' ||
    method === 'cash' ||
    method.includes('cash') ||
    method.includes('collection') ||
    method.includes('lab') ||
...
const createLabTest = async (data) => {
  let labPartnerId = data.lab_partner_id;
  let labName = data.lab;

  if (labPartnerId && !labName) {
    const partner = await prisma.labPartner.findUnique({ where: { id: labPartnerId } });
    if (partner) labName = partner.name;
  }

  if (!labPartnerId) {
    const firstLab = await prisma.labPartner.findFirst({ where: { status: 'approved' } }) || await prisma.labPartner.findFirst();
    if (firstLab) {
      labPartnerId = firstLab.id;
      labName = labName || firstLab.name;
    }
  }
  ...
  const test = await prisma.labTest.create({
    data: {
      name: data.name,
      lab: labName || 'Central Diagnostic Lab',
      lab_partner_id: labPartnerId || null,
      category: data.category || 'blood',
      tests_included: Number(data.tests_included) || 1,
      collection_time: data.collection_time || '30 mins',
      report_time: data.report_time || '24 Hours',
      price: Number(data.price) || 1000,
      popular: Boolean(data.popular),
      home_collection: data.home_collection !== false,
      fasting_required: Boolean(data.fasting_required),
      preparation: data.preparation || null,
      description: data.description || null,
      is_active: true,
    },
    include: {
      lab_partner: { select: { id: true, name: true, city: true, rating: true } }
    }
  });
...
const updateLabTest = async (id, data) => {
  ...
  const updated = await prisma.labTest.update({
    ...
    data: {
      ...
      ...(data.tests_included !== undefined && { tests_included: Number(data.tests_included) }),
      ...(data.collection_time && { collection_time: data.collection_time }),
      ...(data.report_time && { report_time: data.report_time }),
      ...(data.price !== undefined && { price: Number(data.price) }),
      ...(data.popular !== undefined && { popular: Boolean(data.popular) }),
      ...(data.home_collection !== undefined && { home_collection: Boolean(data.home_collection) }),
      ...(data.fasting_required !== undefined && { fasting_required: Boolean(data.fasting_required) }),
