...
router.get('/customers', catchAsync(async (req, res) => {
  const customers = await prisma.user.findMany({
    ...
    select: {
      ...
      addresses: true,
      created_at: true,
      orders: { select: { id: true, total_amount: true, status: true, created_at: true } },
...
router.post('/products', catchAsync(async (req, res) => {
  let { vendor_id, price, retail_price, stock, ...restData } = req.body;

  if (!vendor_id) {
    const vendor = await prisma.vendor.findFirst({ where: { status: 'approved' } }) || await prisma.vendor.findFirst();
    if (!vendor) {
      return res.status(400).json({ status: 'fail', message: 'No active vendor found. Please register/create a vendor first.' });
    }
    vendor_id = vendor.id;
  }

  const numPrice = Number(price) || 0;
  const numRetailPrice = retail_price !== undefined && retail_price !== '' ? Number(retail_price) : numPrice;
  const numStock = Number(stock) || 0;

  const product = await productsService.createProduct(vendor_id, {
    ...restData,
    price: numPrice,
  ...
  res.status(201).json({ status: 'success', data: { product: updated } });
}));
...
router.post('/vendors', catchAsync(async (req, res) => {
  ...
  if (cleanLicense && cleanLicense.toUpperCase() !== 'PENDING') {
    const existingWithLicense = await prisma.vendor.findFirst({
      where: {
        license_number: {
          equals: cleanLicense,
          mode: 'insensitive',
    ...
    if (existingWithLicense) {
      return res.status(400).json({
        status: 'error',
        message: `License number '${cleanLicense}' is already registered to another pharmacy (${existingWithLicense.business_name || existingWithLicense.name}).`,
      });
    }
  }
    ...
    data: {
      ...
      vendor: {
        create: {
          business_name,
          email: normalizedEmail,
          license_number: cleanLicense || 'PENDING',
...
const handleUpdateVendor = catchAsync(async (req, res) => {
  ...
  const vendor = await prisma.vendor.findFirst({
    where: {
      OR: [
        ...
        { business_name: { equals: cleanId, mode: 'insensitive' } },
      ],
    },
  ...
  if (business_name !== undefined) vendorUpdateData.business_name = business_name.trim();
  if (license_number !== undefined) {
    const cleanLicense = (license_number || '').trim();
    if (cleanLicense && cleanLicense.toUpperCase() !== 'PENDING' && cleanLicense.toLowerCase() !== (vendor.license_number || '').toLowerCase()) {
      const existingWithLicense = await prisma.vendor.findFirst({
        where: {
          id: { not: vendor.id },
          license_number: {
    ...
    vendorUpdateData.license_number = cleanLicense;
  }
  ...
  if (iban !== undefined) vendorUpdateData.iban = iban || null;
  if (payout_schedule !== undefined) vendorUpdateData.payout_schedule = payout_schedule || null;
  if (address !== undefined) vendorUpdateData.address = address || null;
  if (city !== undefined) vendorUpdateData.city = city || null;
  if (province !== undefined) vendorUpdateData.province = province || null;
  if (postal_code !== undefined) vendorUpdateData.postal_code = postal_code || null;
  if (phone !== undefined) vendorUpdateData.phone = phone ? phone.trim() : null;
  if (whatsapp !== undefined) vendorUpdateData.whatsapp = whatsapp ? whatsapp.trim() : null;
  if (logo_url !== undefined) vendorUpdateData.logo_url = logo_url || null;
  ...
  if (status !== undefined) vendorUpdateData.status = status;
  if (is_open !== undefined) vendorUpdateData.is_open = Boolean(is_open);
  if (is_online !== undefined) vendorUpdateData.is_online = Boolean(is_online);
...
router.patch('/vendors/:id', handleUpdateVendor);
router.put('/vendors/:id', handleUpdateVendor);
router.patch('/vendors/:id/credentials', handleUpdateVendor);
...
const handleUpdateDoctor = catchAsync(async (req, res) => {
...
router.patch('/doctors/:id', handleUpdateDoctor);
...
router.get('/labs', catchAsync(async (req, res) => {
  ...
  res.json({ status: 'success', data: { labs: mappedLabs } });
}));
...
router.post('/labs', catchAsync(async (req, res) => {
  ...
  const cleanLicense = (license_number || '').trim();
  if (cleanLicense && cleanLicense.toUpperCase() !== 'PENDING') {
    const existingWithLicense = await prisma.labPartner.findFirst({
      where: {
        license_number: { equals: cleanLicense, mode: 'insensitive' },
      },
      select: { id: true, name: true, license_number: true },
    });

    if (existingWithLicense) {
      return res.status(400).json({
        status: 'error',
        message: `License / registration number '${cleanLicense}' is already registered to another lab (${existingWithLicense.name}).`,
      });
    }
  }
...
const handleUpdateLab = catchAsync(async (req, res) => {
  const cleanId = String(req.params.id || '').trim();
  const {
    name,
    license_number,
  ...
  if (!existingLab) {
    return res.status(404).json({ status: 'error', message: 'Lab partner not found' });
  }

  const cleanLicense = license_number !== undefined ? (license_number || '').trim() : undefined;
  if (cleanLicense && cleanLicense.toUpperCase() !== 'PENDING') {
    const duplicate = await prisma.labPartner.findFirst({
      where: {
        id: { not: existingLab.id },
        license_number: { equals: cleanLicense, mode: 'insensitive' },
      },
    ...
    if (duplicate) {
      return res.status(400).json({
        status: 'error',
        message: `License / registration number '${cleanLicense}' is already registered to another lab (${duplicate.name}).`,
      });
    }
  }

  const labUpdateData = {};
  if (name !== undefined) labUpdateData.name = name.trim();
  if (cleanLicense !== undefined) labUpdateData.license_number = cleanLicense;
  if (address !== undefined) labUpdateData.address = address ? address.trim() : null;
  if (city !== undefined) labUpdateData.city = city ? city.trim() : null;
  if (phone !== undefined) labUpdateData.phone = phone ? phone.trim() : null;
  if (bio !== undefined) labUpdateData.bio = bio ? bio.trim() : null;
  if (home_collection !== undefined) labUpdateData.home_collection = Boolean(home_collection);
  if (operating_hours !== undefined) labUpdateData.operating_hours = operating_hours;
  if (collection_areas !== undefined) labUpdateData.collection_areas = collection_areas;
  if (status !== undefined) labUpdateData.status = status;

  const bcrypt = require('bcryptjs');
  const normalizedEmail = email ? email.trim().toLowerCase() : null;

  if (existingLab.account_id) {
    const accountUpdateData = {};
    if (normalizedEmail) accountUpdateData.email = normalizedEmail;
    if (password) accountUpdateData.password = await bcrypt.hash(password, 12);

    if (normalizedEmail && normalizedEmail !== existingLab.account?.email) {
      const emailTaken = await prisma.account.findUnique({ where: { email: normalizedEmail } });
      if (emailTaken && emailTaken.id !== existingLab.account_id) {
        return res.status(400).json({ status: 'error', message: 'Email already in use' });
      }
    }
    ...
    if (Object.keys(accountUpdateData).length > 0) {
      await prisma.account.update({
        where: { id: existingLab.account_id },
        data: accountUpdateData,
      });
    }

    if (normalizedEmail) labUpdateData.email = normalizedEmail;
  } else {
    if (normalizedEmail) labUpdateData.email = normalizedEmail;
    if (password) labUpdateData.password = await bcrypt.hash(password, 12);
  }
  ...
  try {
    await prisma.auditLog.create({
      data: { action: 'LAB_UPDATED', entity: 'lab', entity_id: existingLab.id, user_id: req.user.id }
    });
  } catch {}
  ...
  res.json({
    ...
    data: {
      lab: {
        ...lab,
        email: lab.account?.email || lab.email,
      },
    },
  });
});

router.patch('/labs/:id', handleUpdateLab);
router.put('/labs/:id', handleUpdateLab);
...
router.patch('/labs/:id/status', catchAsync(async (req, res) => {
  ...
  const lab = await prisma.labPartner.update({ where: { id: existingLab.id }, data: { status } });
  
  if (note) {
    try {
      await prisma.auditLog.create({
        data: { action: `LAB_${status.toUpperCase()}`, entity: 'lab', entity_id: existingLab.id, details: { note }, user_id: req.user.id }
      });
    } catch {}
  }
...
router.delete('/labs/:id', catchAsync(async (req, res) => {
  ...
  await prisma.labPartner.delete({ where: { id: existingLab.id } });
  if (existingLab.account_id) {
    try {
      await prisma.account.delete({ where: { id: existingLab.account_id } });
    } catch {}
  }

  try {
    await prisma.auditLog.create({
      data: { action: 'LAB_DELETED', entity: 'lab', entity_id: existingLab.id, user_id: req.user.id }
    });
  } catch {}
