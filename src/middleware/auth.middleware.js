...
const protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentAccount && decoded.role === 'doctor') {
    ...
    if (doctorProfile?.account) {
      currentAccount = doctorProfile.account;
    }
  }

  if (!currentAccount && decoded.role === 'lab') {
    const labProfile = await prisma.labPartner.findUnique({
      where: { id: decoded.id },
      include: {
        account: {
          include: {
    ...
    if (labProfile?.account) {
      currentAccount = labProfile.account;
    }
  }
